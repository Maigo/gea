...
inline quaternion::quaternion(float angle, const vector3 &axis) { from_axis(angle, axis); }
...
inline quaternion::quaternion(float heading, float attitude, float bank) { from_euler(heading, attitude, bank); }
...
// member access
inline float &quaternion::operator[] (int32_t i) {
    assert(mth::range(i,0,3) && "index out of bounds!");
    return (&x)[i];
}
inline const float &quaternion::operator[] (int32_t i) const {
    assert(mth::range(i,0,3) && "index out of bounds!");
    return (&x)[i];
}

// linear algebra
inline float quaternion::length()    const { return sqrtf(x*x + y*y + z*z + w*w); }
inline float quaternion::length_sq() const { return      (x*x + y*y + z*z + w*w); }
...
// attributes
inline bool quaternion::is_zero() const { return (*this) == quaternion::ZERO; }
...
inline bool approx_eq(const quaternion &q1, const quaternion &q2, float e) {
...
inline bool approx_ne(const quaternion &q1, const quaternion &q2, float e) {
    if (q1.dot_product(q2) > 0)
