...
void matrix3::to_quaternion(quaternion &q) const {
    q.from_matrix((*this));
}
...
void matrix3::from_euler(float heading, float attitude, float bank) {
...
void matrix3::to_euler(float &heading, float &attitude, float &bank) {
    // m20 in [~-1,~1]
    if ((m[1][0] > mth::upb(-1.0f)) && (m[1][0] < mth::lwb(1.0f))) {
        heading = atan2f(-m[2][0],m[0][0]);
        // force m10 to [-1,1]
	    attitude = asinf(mth::clamp(m[1][0],-1.0f,1.0f));
    	bank = atan2f(-m[1][2],m[1][1]);
    // singularity at south or north pole
    } else {
        heading = atan2f(m[0][2],m[2][2]);
        attitude = (m[1][0] < 0.0f) ? -PI/2.0f : PI/2.0f;
        bank = 0;
    }
}
