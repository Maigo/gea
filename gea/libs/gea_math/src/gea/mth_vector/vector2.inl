...
// member access
inline float &vector2::operator[] (int32_t i) {
    assert(mth::range(i,0,1) && "index out of bounds!");
    return (&x)[i];
}
inline const float &vector2::operator[] (int32_t i) const {
    assert(mth::range(i,0,1) && "index out of bounds!");
    return (&x)[i];
}
...
// attributes
inline bool vector2::is_zero() const { return (*this) == vector2::ZERO; }
...
inline bool approx_eq(const vector2 &v0, const vector2 &v1, float e) {
    return approx_eq(v0.x, v1.x, e) &&
           approx_eq(v0.y, v1.y, e);
}
inline bool approx_ne(const vector2 &v0, const vector2 &v1, float e) {
