...
public:
	...
	late_constructed() GEA_NOEXCEPT // In the case of the late_constructed instance being at global scope, we rely on the 
	...
	bool is_constructed() const GEA_NOEXCEPT {
        return mpValue != NULL;
    }
    ...
    value_type& operator*() GEA_NOEXCEPT
	{
        if (!mpValue) {
            construct();
        }
...
public:
	...
	typename base_type::value_type& operator*() GEA_NOEXCEPT {
        GEA_ASSERT(base_type::mpValue);
	...
	typename base_type::value_type* get() GEA_NOEXCEPT {
        return base_type::mpValue;
    }
};
