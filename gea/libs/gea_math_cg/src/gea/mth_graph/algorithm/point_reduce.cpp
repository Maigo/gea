...
void point_reduce_rdp::point_reduce(const std::vector<point2> &points, const float epsilon, std::vector<point2> &out_points)
{
    ...
    stack stack;
    stack.push_back(gea_compound(stack_item){ 0u, points.size() - 1u });
    ...
    while (!stack.empty()) {
        ...
        if (distance > epsilon) {
            stack.push_back(gea_compound(stack_item){ index, args.to });
            stack.push_back(gea_compound(stack_item){ args.from, index });
        } else {
            // include start point
